...
int numberSetting = 1;
char hexToInt = 0;
char currentNumber[] = "0000";

void setHexToChar(int numberFromKey, int NumberSetting)
{
  currentNumber[numberSetting - 1] = numberFromKey + '0';
  Serial.print("setHex ");
  Serial.println(currentNumber);
}
void setNumber(uint64_t currentKey)
{
  if (last_key != currentKey)
  {
    last_key = currentKey;
    ...
    case (uint64_t)KEY_BUTTON::TIMESHIFT:
      Serial.println("Dang chay timeShift");
      numberSetting = 2;
      break;
    case (uint64_t)KEY_BUTTON::I1:

      setHexToChar(1, numberSetting);
      break;
    case (uint64_t)KEY_BUTTON::I2:
      setHexToChar(2, numberSetting);
      break;
    case (uint64_t)KEY_BUTTON::I3:
      setHexToChar(3, numberSetting);
      break;
    case (uint64_t)KEY_BUTTON::I4:
      setHexToChar(4, numberSetting);
      break;
    case (uint64_t)KEY_BUTTON::I5:
      setHexToChar(5, numberSetting);
      break;
    case (uint64_t)KEY_BUTTON::I6:
      setHexToChar(6, numberSetting);
      break;
    case (uint64_t)KEY_BUTTON::I7:
      setHexToChar(7, numberSetting);
      break;
    case (uint64_t)KEY_BUTTON::I8:
      setHexToChar(8, numberSetting);
      break;
    case (uint64_t)KEY_BUTTON::I9:
      setHexToChar(9, numberSetting);
      break;
    case (uint64_t)KEY_BUTTON::I0:
      setHexToChar(0, numberSetting);
      break;
    case (uint64_t)KEY_BUTTON::RIGHT:
    {
      Serial.println("So hien tai la");
      numberSetting =4;

      if(numberSetting>4)
        numberSetting = 4;

      Serial.println(numberSetting);
    }
    case (uint64_t)KEY_BUTTON::LEFT:
      if (numberSetting == 0)
        numberSetting = 4;
      else
        numberSetting -= 1;
  ...
  //thiet dat vi tri dau tien nhap tu tu
  //0000 voi 0 dau tien nhap nhay
  ...
  if (togle)
  {
    write(bitmap_table[currentNumber[numberSetting - 1] - '0'], numberSetting);
  }
  else
  {
    write(bitmap_table[10], numberSetting);

  }
  delay(povDelay);
  for (int i = 0; i < 4; i++)
  {
    if (i + 1 != numberSetting)
    {
      write(bitmap_table[currentNumber[i] - '0'], i + 1);
